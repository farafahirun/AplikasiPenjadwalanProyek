...
class ProjectSchedulerApp:
    def __init__(self, root):
        ...
        self.input_frame = ttk.Frame(self.main_pane, width=380)
    ...
    def create_input_widgets(self):
        ...
        input_data_frame = ttk.LabelFrame(self.input_frame, text="Input Data Kegiatan")
        input_data_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=5)
        input_data_frame.columnconfigure(1, weight=1)

        ttk.Label(input_data_frame, text="Nama Kegiatan:").grid(row=0, column=0, padx=10, pady=8, sticky=tk.W)
        self.name_entry = ttk.Entry(input_data_frame)
        self.name_entry.grid(row=0, column=1, padx=10, pady=8, sticky=tk.EW)

        ttk.Label(input_data_frame, text="Durasi (hari):").grid(row=1, column=0, padx=10, pady=8, sticky=tk.W)
        self.duration_entry = ttk.Entry(input_data_frame)
        self.duration_entry.grid(row=1, column=1, padx=10, pady=8, sticky=tk.EW)

        ttk.Label(input_data_frame, text="Dependensi:").grid(row=2, column=0, padx=10, pady=8, sticky=tk.W)
        self.deps_entry = ttk.Entry(input_data_frame)
        self.deps_entry.grid(row=2, column=1, padx=10, pady=8, sticky=tk.EW)

        self.button_frame = ttk.Frame(input_data_frame)
        self.button_frame.grid(row=3, column=0, columnspan=2, pady=10)
        self.create_standard_buttons()
        
        file_frame = ttk.LabelFrame(self.input_frame, text="Aksi File")
        file_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=10)
        file_frame.columnconfigure(0, weight=1)
        ttk.Button(file_frame, text="Impor dari Excel", command=self.import_from_excel).grid(sticky="ew", padx=10, pady=5, ipady=4)
        ttk.Button(file_frame, text="Ekspor Hasil ke Excel", command=self.export_results).grid(sticky="ew", padx=10, pady=5, ipady=4)

        process_frame = ttk.LabelFrame(self.input_frame, text="Pengolahan Proyek")
        process_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=10)
        process_frame.columnconfigure(0, weight=1)

        self.run_button = tk.Button(process_frame, text="HITUNG & BUAT DIAGRAM", 
                                    font=("Segoe UI", 11, "bold"), fg="white", bg="#005A9E",
                                    activebackground="#004578", activeforeground="white",
                                    command=self.run_analysis, relief="flat", borderwidth=0)
        self.run_button.grid(sticky="ew", padx=10, pady=10, ipady=8)
    ...
    def create_standard_buttons(self):
        ...
        ttk.Button(self.button_frame, text="Hapus", command=self.delete_selected_activity).pack(side=tk.LEFT, padx=5)
    ...
    def create_edit_buttons(self):
        ...
        save_button = tk.Button(self.button_frame, text="Simpan Perubahan",
                                font=("Segoe UI", 10, "bold"), fg="white", bg="#005A9E",
                                activebackground="#004578", activeforeground="white",
                                command=self.add_or_update_activity, relief="flat", borderwidth=0, padx=10)
        save_button.pack(side=tk.LEFT, padx=10, ipady=2)

        cancel_button = tk.Button(self.button_frame, text="Batal",
                                  font=("Segoe UI", 10), fg="black", bg="#E1E1E1",
                                  activebackground="#C8C8C8", activeforeground="black",
                                  command=self.exit_edit_mode, relief="flat", borderwidth=0, padx=10)
        cancel_button.pack(side=tk.LEFT, padx=10, ipady=2)
    ...
    def create_output_widgets(self):
        ...
        table_frame = ttk.LabelFrame(self.output_frame, text="Tabel Daftar Kegiatan")
        table_frame.grid(row=0, column=0, sticky="nsew", padx=(0,10), pady=5)
        ...
        self.notebook.grid(row=1, column=0, sticky="nsew", padx=(0,10), pady=10)
    ...
    def create_diagram_legend(self, parent_frame):
        ...
        legend_frame.pack(pady=(5, 10), padx=5, fill=tk.X)
    ...
    def draw_network_diagram(self):
        for widget in self.network_tab.winfo_children():
            widget.destroy()
        if not hasattr(self, 'results_df'): return
        container = ttk.Frame(self.network_tab)
        container.pack(fill=tk.BOTH, expand=True)
        ...
        for node, (x, y) in pos.items():
            ...
            ax.text(x, y, label, ha='center', va='center',
                    fontsize=9, bbox=props, fontfamily='monospace', zorder=10)
        ...
        ax.set_title("Network Diagram Proyek", fontsize=16, fontweight='bold')
        ax.axis('off')
        plt.tight_layout()
    ...
    def draw_gantt_chart(self):
        ...
        colors = ['#D83B01' if row['Nama Kegiatan'] in self.critical_path_names else '#0078D7' for _, row in df.iterrows()]
        ax.barh(df['Nama Kegiatan'], df['Durasi'], left=df['ES'], color=colors, edgecolor='black', zorder=3)
        ax.set_xlabel("Durasi Proyek (Hari)", fontweight='bold')
        ax.set_ylabel("Nama Kegiatan", fontweight='bold')
        ax.set_title("Gantt Chart Proyek", fontsize=15, fontweight='bold')
        ax.grid(True, which='major', axis='x', linestyle='--', linewidth=0.5, zorder=0)
        ax.invert_yaxis()
        for index, row in df.iterrows():
            ax.text(row['ES'] + row['Durasi']/2, index, str(int(row['Durasi'])), va='center', ha='center', color='white', fontweight='bold', fontsize=10)
        plt.tight_layout()
        self.gantt_fig = fig
